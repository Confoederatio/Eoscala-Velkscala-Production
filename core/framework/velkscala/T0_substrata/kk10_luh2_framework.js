//Initialise functions
{
	...
	global.generateKK10_LUH2Rasters = function () {
		...
		for (var i = 0; i < hyde_years.length; i++) try {
			...
			var output_file_path = `${common_defines.kk10luh2_folder}/${common_defines.kk10luh2_prefix}${i}.png`;

			if (in_luh2_domain || in_kk10_domain) {
				//1. If this is an intersection of both the luh2_domain and kk10_domain; average rasters
				var luh2_file_path = `${luh2_config.output_folder}/${luh2_config.file_prefix}${i}.png`;
				var kk10_file_path = `${kk10_config.input_folder}/${kk10_config.file_prefix}${i}.png`;
				...
				if (in_luh2_domain && in_kk10_domain) {
					...
					log.info(`- Averaging KK10 and LUH2 for ${i} ..`);
					saveNumberRasterImage({
						file_path: output_file_path,
						type: "greyscale",
