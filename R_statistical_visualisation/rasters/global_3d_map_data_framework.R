...
generate_global_3d_map <- function(data_png_path, region_key, options = list()) {
  library(png)
  library(plotly)
  library(V8)
  ...
  region_mask_file <- "./input/eoscala/land_sea.png"
  regions_definitions_file <- "./common/eoscala/history/land_sea.js"
  ...
  regions_js_content <- readLines(regions_definitions_file)
  ctx$eval(paste(regions_js_content, collapse = "\n"))
  regions_data <- ctx$get("config.eoscala.history.land_sea")
    ...
    for (j in 2:(width - 1)) {
      if (mask[i, j] == 1L) {
        ...
        if (!is.na(neighbor_avg) && neighbor_avg > 0 &&
            heightmap[i, j] > 100*neighbor_avg) { # Remove outliers 100x greater than the average of its neighbours
  ...
  tick_vals_original <- generate_log_ticks(valid_min_gdp, max_gdp, n = 11)
  tick_positions <- seq(0, 100, by = 10)
  tick_labels <- sapply(tick_vals_original, function(val) abbreviate_number(val, prefix = opts$z_label_prefix))
  ...
  # Mask-aware downscaling helpers
  downscale_matrix <- function(mat, factor) {
    if (factor <= 1) return(mat)
    rows <- seq(1, nrow(mat), by = factor)
    cols <- seq(1, ncol(mat), by = factor)
    mat[rows, cols, drop = FALSE]
  }
  
  downscale_vector <- function(vec, factor) {
    if (factor <= 1) return(vec)
    vec[seq(1, length(vec), by = factor)]
  }
  
  # Downscale factor (default = 2)
  ds_factor <- opts$downscale_factor %||% 2
  
  # Apply mask first to ensure only valid region data
  mask_na <- ifelse(mask == 1L, 1, NA_real_)
  z_matrix_masked <- z_matrix_final * mask_na
  
  # Extract region using masked matrix
  z_region <- z_matrix_masked[y0:y1, x0:x1]
  x_coords <- x0:x1
  y_coords <- y0:y1
  
  # Now downscale
  z_region <- downscale_matrix(z_region, ds_factor)
  x_coords <- downscale_vector(x_coords, ds_factor)
  y_coords <- downscale_vector(y_coords, ds_factor)
